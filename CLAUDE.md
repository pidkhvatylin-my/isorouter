...
- `#commit` is the async state machine: abort any in-flight commit → `matchRoutes` → run `beforeLoad` guards **root→leaf** → resolve (lazy) components → build `metadata` (shallow-merge `RouteMetadata` root→leaf, child keys override parent keys) → `#emit` snapshot. `RouterStatus` = `idle | navigating | not-found | error`. Core has **zero DOM side effects** — v2 removed the old "apply title" step entirely; `snapshot.metadata` is just carried data, and applying it (`document.title`, a head library) is the app's job, typically via `onCommit`.
