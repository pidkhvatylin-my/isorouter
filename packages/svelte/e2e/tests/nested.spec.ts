...
test("renders a layout's index child and applies the layout's title via `onCommit`", async ({
  page,
}) => {
  await page.goto("/");
...
test("renders a nested child inside its parent's outlet and applies the layout's title via `onCommit`", async ({
  page,
}) => {
  await page.goto("/");
